...
function AHCCData:checkVersion(obj)
    if obj.versionStart and obj.versionEnd then
        return (tonumber(obj.versionStart) <= tonumber(AHCC.gameVersion)) and
                   (tonumber(obj.versionEnd) > tonumber(AHCC.gameVersion))
...
function AHCCData:add(data, mode)
    if data.Config then
        ...
        if not checkExpansionLevel(data.Config.expansionLevel) then
            return
        end
    end
...
AHCC_DATA["OnEnable"] = function(self)
    -- AHCCData:add(self)
    table.insert(DataTable, {data = self})
    self.loaded = true
...
function AHCC_DATA__checkModules()
    ...
    local check = _.every(modules, function(module) return module.loaded end)

    if AHCC_DATA__ticker then AHCC_DATA__ticker:Cancel() end

    if check then
        AHCC_DATA__ticker = C_Timer.NewTimer(0.01, function()
            AHCC_DATA__ticker:Cancel()

            DataTable = AHCCCustom:inject(DataTable)

            local insertData = _.filter(DataTable, function(data)
                return data.data.Config.mode == nil or data.data.Config.mode ==
                           "insert"
            end)

            -- DevTool:AddData(insertData)

            _.forEach(insertData,
                      function(data)
            ...
            _.forEach(mergeData,
                      function(data) AHCCData:add(data.data, "merge") end)
...
local function deep_equals(o1, o2, ignore_mt)
    ...
    for key2, _ in pairs(o2) do if o1[key2] == nil then return false end end
    return true
end
